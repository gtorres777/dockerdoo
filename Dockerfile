...
RUN set -x; \
    pip3.9 -qq install --no-cache-dir --upgrade \
    astor \
    black \
    geoip2 \
    ptvsd \
...
ENV \
    ...
    # Python libries path
    PYTHON_PATH=${PYTHON_PATH}:-/home/odoo/.local/lib/python3.9/site-packages
...
RUN git clone --depth=1 -b ${ODOO_VERSION} https://github.com/odoo/odoo.git ${ODOO_BASEPATH} \
    && pip3.9 install -r ${ODOO_BASEPATH}/requirements.txt \
